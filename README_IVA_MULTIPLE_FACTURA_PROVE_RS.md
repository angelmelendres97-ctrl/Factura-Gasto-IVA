...
El guardado del detalle de IVA múltiple valida primero si la tabla `saefprv_iva_det` existe en la base Informix mediante `systables`. Si la tabla aún no fue creada en un ambiente, la factura puede guardarse con los campos históricos consolidados de `saefprv` sin romper el flujo transaccional. En ambientes donde sí exista la tabla, cualquier error al borrar o insertar el detalle devuelve `false` y detiene el guardado para evitar que el detalle por porcentaje quede inconsistente.

También se escapan los valores alfanuméricos usados en el `delete/insert` (`factura`, `serie` y `usuario`) y la fecha de servidor se inserta como `current year to second`, alineada con el tipo `datetime year to second` propuesto para `fpid_fech_server`.
