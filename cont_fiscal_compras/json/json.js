...
function escaparHtmlExcelCompras(valor) {
    return String(valor)
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
...
function descargarExcelRetencionesCompras(e, dt) {
    var columnas = [
        {titulo: 'RUC', campo: 'ruc'},
        {titulo: 'Razón Social', campo: 'razon_social'},
        {titulo: 'Fecha', campo: 'fecha'},
        {titulo: 'Tipo', campo: 'tipo'},
        {titulo: 'Serie', campo: 'serie'},
        {titulo: 'Secuencial', campo: 'secuencial'},
        {titulo: 'Detalle', campo: 'detalle'},
        {titulo: 'VALOR 15%', campo: 'valor_15'},
        {titulo: 'VALOR 5%', campo: 'valor_5'},
        {titulo: 'VALOR 0%', campo: 'valor_0'},
        {titulo: 'IVA 15%', campo: 'iva_15'},
        {titulo: 'IVA 5%', campo: 'iva_5'},
        {titulo: 'Total Factura', campo: 'total_factura'},
        {titulo: 'Código Retención', campo: 'codigo_retencion', retencion: true},
        {titulo: 'No. Retención', campo: 'numero_retencion', retencion: true},
        {titulo: 'Valor Base Retención', campo: 'valor_base_retencion', retencion: true},
        {titulo: 'Valor Retenido', campo: 'valor_retenido', retencion: true},
        {titulo: 'Total a Pagar', campo: 'total_pagar'},
        {titulo: 'Asiento Contable', campo: 'asiento_contable'}
    ];
    var datos = dt.rows({search: 'applied'}).data().toArray();
    var html = '';

    html += '<html><head><meta charset="UTF-8">';
    html += '<style>';
    html += 'table{border-collapse:collapse;font-family:Arial;font-size:10pt;}';
    html += 'th{background:#D9EAF7;font-weight:bold;text-align:center;}';
    html += 'td,th{border:1px solid #000;padding:4px;vertical-align:middle;mso-number-format:"\\@";}';
    html += '.num{text-align:right;}';
    html += '</style></head><body>';
    html += '<table><thead><tr>';

    for (var h = 0; h < columnas.length; h++) {
        html += '<th>' + columnas[h].titulo + '</th>';
    }
    html += '</tr></thead><tbody>';

    for (var i = 0; i < datos.length; i++) {
        var fila = datos[i];
        var codigos = obtenerLineasExportacionCompras(fila.codigo_retencion);
        var numeros = obtenerLineasExportacionCompras(fila.numero_retencion);
        var bases = obtenerLineasExportacionCompras(fila.valor_base_retencion);
        var retenidos = obtenerLineasExportacionCompras(fila.valor_retenido);
        var maxLineas = Math.max(codigos.length, numeros.length, bases.length, retenidos.length);
        ...
        for (var linea = 0; linea < maxLineas; linea++) {
            ...
            for (var c = 0; c < columnas.length; c++) {
                ...
                if (columna.campo === 'codigo_retencion') {
                    valor = codigos[linea] || '';
                } else if (columna.campo === 'numero_retencion') {
                    valor = numeros[linea] || '';
                } else if (columna.campo === 'valor_base_retencion') {
                    valor = bases[linea] || '';
                } else if (columna.campo === 'valor_retenido') {
                    valor = retenidos[linea] || '';
                } else {
                    if (linea > 0) {
                        continue;
                    }
                    valor = formatearCeldaExportacionCompras(fila[columna.campo]);
                }

                if (!columna.retencion && maxLineas > 1) {
                    html += '<td rowspan="' + maxLineas + '"' + (c >= 7 && c <= 17 ? ' class="num"' : '') + '>' + escaparHtmlExcelCompras(valor) + '</td>';
                } else {
                    html += '<td' + (c >= 7 && c <= 17 ? ' class="num"' : '') + '>' + escaparHtmlExcelCompras(valor) + '</td>';
                }
            }
            html += '</tr>';
        }
    ...
    enlace.href = url;
    enlace.download = 'reporte_compras_iva_multiple_' + fecha + '.xls';
...
function consultarJsonNuevo(opFactura) {
    ...
    $('#data-table').DataTable({
        ...
        "buttons": [
            {
                text: 'Excel Retenciones',
                action: descargarExcelRetencionesCompras
            },
            {
                extend: 'excelHtml5',
                autoFilter: true,
