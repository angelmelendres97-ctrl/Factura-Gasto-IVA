...
7. El usuario puede exportar a Excel, CSV, impresión o PDF horizontal desde los botones de DataTables. En Excel, las facturas con varias retenciones se expanden en filas secundarias para que códigos, bases y valores retenidos no se descarguen concatenados; en PDF se conservan saltos de línea dentro de la celda.
...
- Exportar a Excel y verificar que las columnas `Código Retención`, `Valor Base Retención` y `Valor Retenido` generen una fila por retención cuando existan varios valores, dejando en blanco las demás columnas en las filas secundarias.
