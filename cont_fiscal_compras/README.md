...
7. El usuario puede exportar a Excel, CSV, impresión o PDF horizontal desde los botones de DataTables. Para retenciones múltiples se debe usar el botón **Excel Retenciones**, que genera un archivo `.xls` con filas secundarias y celdas combinadas (`rowspan`) para que códigos, bases y valores retenidos no se descarguen concatenados; en PDF se conservan saltos de línea dentro de la celda.
