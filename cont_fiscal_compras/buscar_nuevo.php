...
try {
    ...
    if (!empty($sucursal)) {
        ...
        $tmpSucursalMinv = " and m.minv_cod_sucu = $sucursal";
    }
    ...
    if ($opFactura == 1 || $opFactura == 2) {
        $sql = "select m.minv_num_comp, c.clpv_ruc_clpv, c.clpv_nom_clpv, m.minv_fmov,
                       m.minv_fac_prov, m.minv_ser_docu, m.minv_cm1_minv,
                       m.minv_cod_clpv, m.minv_cod_tran, m.minv_cod_sucu,
                       m.minv_cod_ejer, m.minv_num_prdo, m.minv_comp_cont,
                       round(((COALESCE(m.minv_iva_valo, 0) * 100) / 15), 2) as valor_15,
                       0 as valor_5,
                       round(((m.minv_tot_minv - COALESCE(m.minv_dge_valo, 0) + COALESCE(m.minv_otr_valo, 0) + COALESCE(m.minv_fle_minv, 0)) - round(((COALESCE(m.minv_iva_valo, 0) * 100) / 15), 2)), 2) as valor_0,
                       round(COALESCE(m.minv_iva_valo, 0), 2) as iva_15,
                       0 as iva_5,
                       round((m.minv_tot_minv - COALESCE(m.minv_dge_valo, 0) + COALESCE(m.minv_otr_valo, 0) + COALESCE(m.minv_fle_minv, 0) + COALESCE(m.minv_val_ice, 0) + COALESCE(m.minv_iva_valo, 0)), 2) as total_factura
                from saeminv m, saeclpv c
                where m.minv_cod_empr = c.clpv_cod_empr and
                      ...
                      $tmpSucursalMinv
                order by m.minv_fmov, m.minv_ser_docu, m.minv_fac_prov";

        if ($oIfx->Query($sql)) {
            if ($oIfx->NumFilas() > 0) {
                do {
                    $codSucu = $oIfx->f('minv_cod_sucu');
                    $codAsto = $oIfx->f('minv_comp_cont');
                    $codEjer = $oIfx->f('minv_cod_ejer');
                    $numPrdo = $oIfx->f('minv_num_prdo');
                    $codTran = $oIfx->f('minv_cod_tran');
                    $codClpv = $oIfx->f('minv_cod_clpv');
                    $numFact = limpiar_reporte_compras_nuevo($oIfx->f('minv_fac_prov'));
                    $fecha = $oIfx->f('minv_fmov');
                    $numComp = $oIfx->f('minv_num_comp');
                    ...
                    foreach ($retenciones as $retencion) {
                        if (!empty($retencion[0])) {
                            $codigosRetencion[] = $retencion[0];
                        }
                        if (!empty($retencion[1]) && !isset($numerosRetencion[$retencion[1]])) {
                            $numerosRetencion[$retencion[1]] = "<span style=\"cursor: pointer; font-weigth: bold;\" onclick=\"genera_documento(6, '" . $numComp . "', 'no_autorizado', '" . $codClpv . "', '" . $numFact . "', '" . $codEjer . "', '" . $codAsto . "', '" . $fecha . "', " . $codSucu . ")\">" . $retencion[1] . "</span>";
                        }
                        if (!empty($retencion[0]) || (float)$retencion[2] != 0) {
                            $basesRetencion[] = numero_reporte_compras_nuevo($retencion[2]);
                        }
                        if (!empty($retencion[0]) || (float)$retencion[3] != 0) {
                            $valoresRetenidos[] = numero_reporte_compras_nuevo($retencion[3]);
                        }
                    }
                    ...
                    agregar_fila_reporte_compras_nuevo($tabla, array(
                        'ruc' => limpiar_reporte_compras_nuevo($oIfx->f('clpv_ruc_clpv')),
                        'razon_social' => limpiar_reporte_compras_nuevo($oIfx->f('clpv_nom_clpv')),
                        'fecha' => $fecha,
                        'tipo' => isset($arrayTran[$codTran]) ? $arrayTran[$codTran] : $codTran,
                        'serie' => limpiar_reporte_compras_nuevo($oIfx->f('minv_ser_docu')),
                        'secuencial' => $numFact,
                        'detalle' => limpiar_reporte_compras_nuevo($oIfx->f('minv_cm1_minv')),
                        'valor_15' => numero_reporte_compras_nuevo($oIfx->f('valor_15')),
                        'valor_5' => numero_reporte_compras_nuevo($oIfx->f('valor_5')),
                        'valor_0' => numero_reporte_compras_nuevo($oIfx->f('valor_0')),
                        'iva_15' => numero_reporte_compras_nuevo($oIfx->f('iva_15')),
                        'iva_5' => numero_reporte_compras_nuevo($oIfx->f('iva_5')),
                        'total_factura' => numero_reporte_compras_nuevo($totalFactura),
                        'codigo_retencion' => implode('<br>', $codigosRetencion),
                        'numero_retencion' => implode('<br>', $numerosRetencion),
                        'valor_base_retencion' => implode('<br>', $basesRetencion),
                        'valor_retenido' => implode('<br>', $valoresRetenidos),
                        'total_pagar' => numero_reporte_compras_nuevo($totalFactura - $totalRetenido),
            ...
            if ($oIfx->NumFilas() > 0) {
                do {
                    ...
                    foreach ($retenciones as $retencion) {
                        ...
                        if (!empty($retencion[1]) && !isset($numerosRetencion[$retencion[1]])) {
                            $numerosRetencion[$retencion[1]] = "<span style=\"cursor: pointer; font-weigth: bold;\" onclick=\"genera_documento(5, '','no_autorizado' , '" . $codClpv . "'  , '" . $numFact . "', '" . $codEjer . "', '" . $codAsto . "',  '" . $fecha . "', " . $codSucu . ")\">" . $retencion[1] . "</span>";
                        }
                        if (!empty($retencion[0]) || (float)$retencion[2] != 0) {
                            $basesRetencion[] = numero_reporte_compras_nuevo($retencion[2]);
                        }
                    ...
                    agregar_fila_reporte_compras_nuevo($tabla, array(
                        ...
                        'fecha' => $fecha,
                        'tipo' => isset($arrayTran[$codTran]) ? $arrayTran[$codTran] : $codTran,
                        'serie' => limpiar_reporte_compras_nuevo($oIfx->f('fprv_num_seri')),
                        'secuencial' => $numFact,
                        'detalle' => limpiar_reporte_compras_nuevo($oIfx->f('fprv_det_fprv')),
                        'valor_15' => numero_reporte_compras_nuevo($oIfx->f('valor_15')),
                        'valor_5' => numero_reporte_compras_nuevo($oIfx->f('valor_5')),
                        'valor_0' => numero_reporte_compras_nuevo($oIfx->f('valor_0')),
                        'iva_15' => numero_reporte_compras_nuevo($oIfx->f('iva_15')),
